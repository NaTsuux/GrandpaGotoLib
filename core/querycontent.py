...
class Reserve(Enum):
    ...
    def fill_reserve_lib_status_query():
        return json.dumps(Reserve.reserve_lib_status_query)

    @staticmethod
    def fill_reserve_seat_status_query(lib_id: int):
        result = Reserve.reserve_seat_status_query.copy()
        result.get("variables").update({"libId": lib_id})
        return json.dumps(result)

    @staticmethod
    def fill_do_reserve_seat_query(seat_key: str, lib_id: int):
        result = Reserve.do_reserve_seat_query.copy()
...
if __name__ == '__main__':
    print(type(Preserve.get_captcha_query) is Preserve)
