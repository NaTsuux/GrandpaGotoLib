...
def pickup():
    ...
    while True:
        ...
        req = session.post(url=url, data=fill_reserve_lib_status_query_attr())
        ...
        for lib_id in free_seat:
            if lib_id == 323:
                continue
            result = session.post(url=url, data=fill_reserve_seat_status_query_attr(lib_id))
            ...
            for seat in seat_list:
                if seat.get('seat_status') == SEAT_AVAILABLE_CODE and check_seat_privilege(lib_id, seat.get('name')):
            ...
            if len(available_seat_list) != 0:
                req = session.post(url=url, data=fill_do_reserve_seat_query_attr(available_seat_list[0].key, lib_id))
...
def pre_reserve():
    ...
    #
    #     if target_time - server_time <= 3:
    #         break
    #
    #     sleep_time = min(target_time - server_time - 3, 30)
    ...
    while True:
        ...
        write_file("pre_get_step_query", req.text)
        ...
        while True:
            # 发送保留座位请求
            req = session.post(url=url, data=fill_do_pre_seat_query("34,20", 323))
            save = get_general_status(req.text).get("prereserve").get("save")
            pre_logger.info("Seat has been taken by others try one more time")
            if save:
